...
새 정책이 합의·변경되면 `.Codex/skills/buddybird-policy-update` skill이 자동 수행:

- 발화 트리거: "정책 변경", "이제부터", "의무화", "from now on", "deprecated" 등
- 명시 호출: `/buddybird-policy-update <한 줄 정책 설명>`

skill이 (1) 해당 카테고리 docs 갱신, (2) `docs/POLICY-HISTORY.md`에 행 추가, (3) `AGENTS.md` 포인터 점검, (4) 검증 grep 실행을 자동으로 처리합니다.

위치: `buddybird-mobile/.Codex/skills/buddybird-policy-update/`
...
- `hotfix/*` → `main`: 긴급 패치, staging 우회 가능. merge 후 `main → staging` back-merge **(핫픽스 한정 의무)**. 일반 release 의 staging version 은 한 사이클 뒤처지는 게 정상 (단일 semver + buildNumber 분리 패턴, `docs/BUILD-AND-RELEASE.md` §12.2)
