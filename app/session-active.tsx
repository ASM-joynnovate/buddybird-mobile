...
function SessionActiveInner({
  ...
  const { exitWithStop, isConfirmVisible, dismissConfirm } = useSessionExit({ status: session.status, stopSession: handleStop, endedExternally: session.endedExternally });
  const fmt = (s: number) =>
