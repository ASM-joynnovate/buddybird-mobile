...
export function creditRecovery(data: AppData, recovery: PendingRecovery): History | undefined {
	...
	if (!draft.metricsCredited) {
		...
		data.settings.wordMetrics[metricsId] = {
			...
			lifetime_recording_count:
				...
				(recovery.reason === "duration-reached" && draft.word.audioUri ? 1 : 0),
			last_practiced_at_iso: endedAt,
		}
		// Short sessions have no history row, so retain their receipt until native clear succeeds.
		data.sessionDrafts[draft.id] = { ...draft, metricsCredited: true }
	}
	...
	const history: History = {
		...
		completedCycles: Math.floor(
			Math.min(elapsedRunningMs / 1000, draft.settings.totalDurationSeconds) /
