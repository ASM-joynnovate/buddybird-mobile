...
export default function SessionSetupScreen() {
  ...
  const startLabel =
    setup.audioSource === 'preset'
      ? setup.selectedEntry
        ? `세션 시작 · "${setup.selectedEntry.label}" · ${setup.totalCycles}사이클`
        : '세션 시작'
    ...
    <PetScreen contentStyle={styles.content}>
      ...
      {setup.audioSource === 'preset' && (
        <PresetWordPicker
          ...
          selectedId={setup.selectedEntryId}
          onSelect={setup.selectEntry}
          getSessionCount={setup.getEntrySessionCount}
        />
      )}
