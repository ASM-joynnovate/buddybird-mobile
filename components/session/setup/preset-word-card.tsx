...
import type { PresetWord } from '@/features/training/session-config';
...
export function PresetWordCard({ word, active, onSelect, sessionCount }: PresetWordCardProps) {
    ...
    <TouchableOpacity activeOpacity={0.8} onPress={onSelect} style={[styles.card, active && styles.cardActive]}>
      <View style={styles.row}>
        <View style={styles.textBlock}>
          <Text style={[styles.phrase, active && styles.phraseActive]}>{word.word}</Text>
          <Text style={[styles.cat, active && styles.catActive]}>{word.cat}</Text>
        </View>
