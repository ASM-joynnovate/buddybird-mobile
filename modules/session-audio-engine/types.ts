...
export interface EvictedCapture {
	fileName: string
	segmentId: string
	sizeBytes: number
	capturedAt: string
	capture?: CapturedSegment
}

export interface CaptureChanges {
	captures: CapturedSegment[]
	evicted: EvictedCapture[]
}

export interface TargetPlayback {
	sessionId: string
	sequence: number
	durationMs: number
