...
iOS 14.5 이상에서는 첫 실행 시 추적 동의를 묻습니다. 동의 대상에는 화면 사용 기록(세션 리플레이), 익명 사용 통계, 광고 식별자(IDFA)가 포함됩니다. **거부하셔도 앱의 모든 기능을 정상 이용**할 수 있으며, 거부 시 위 분석·추적 데이터 수집은 모두 비활성화됩니다(크래시 진단 제외). 동의 결정 전에는 어떠한 분석·추적 데이터도 수집하지 않습니다.
