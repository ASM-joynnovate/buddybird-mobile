...
| 항목 | 수집 도구 | 목적 |
|---|---|---|
| 자유서술 의견 | Cloud Firestore | 제품 개선 및 오류·불편 사항 파악 |
| Firebase 익명 Auth uid | Firebase Authentication, Cloud Firestore | 같은 앱 사용자가 제출한 의견을 pseudonymous하게 연결하고 쓰기 권한 보호 |
| 앱 버전, 플랫폼, 언어 설정, 서버 기록 시각 | Cloud Firestore | 피드백이 발생한 환경 확인 |

같은 uid는 1.1의 분석·오류·세션 리플레이 데이터와 피드백에 공통으로 적용되므로, 서로 다른
서비스의 기록이 동일한 pseudonymous 식별자로 연결될 수 있습니다.

피드백 폼은 보호자의 이름·이메일·전화번호·정밀 위치를 별도 입력 항목으로 요구하지 않으며, 이러한
개인정보를 자유서술 의견에 입력하지 말라는 안내를 표시합니다.

### 1.4 앱이 별도 입력 항목으로 요구하지 않는 정보

- 보호자(사용자) 본인의 실명, 이메일, 전화번호
- 정밀 위치 정보
- 연락처, 사진(앵무새 사진 제외, 디바이스 외부 전송 없음)

단, 사용자가 피드백 자유서술란에 직접 입력한 내용은 제출된 의견에 포함될 수 있습니다.
...
- Cloud Firestore: 사용자가 제출한 피드백과 익명 uid·앱 환경 정보를 Google LLC가 보관
- Microsoft Clarity: Microsoft Corporation이 미국 서버에 보관
- 위 서비스 외 제3자에게 정보를 판매하거나 제공하지 않습니다.
