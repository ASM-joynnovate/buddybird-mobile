...
4. debug 빌드 전용 방해 요소(dev-client 런처·dev menu·업데이트 모달·에뮬레이터 ANR)는 subflow 자가 회복 루프가 정리. subflow의 Metro 런처 진입 스텝은 **Metro 서버 행(`http://10.0.2.2:8081`) 가시성에 조건부(`when:visible`)**다 — debug 빌드엔 그 행이 떠 탭되고, preview/release 빌드(JS 번들 내장)엔 끝내 안 떠 자동 스킵된다. env 플래그 없이 빌드 종류로 자동 분기하므로 로컬(`yarn e2e:android[:smoke]`)·CI 모두 커맨드가 같다. dev menu(Continue/Reload)도 dev-client 전용이라 preview엔 노출되지 않고 `when:visible` 가드로 무시된다 (BB-384)
...
- **빌드**: EAS `preview` 프로필로 Android APK 빌드 → 에뮬레이터 설치. preview는 `developmentClient` 미포함(JS 번들 내장)이라 **Metro 불필요** → subflow의 Metro 런처 진입 스텝이 가시성 조건부로 자동 스킵됨 (§8.3 step 4). dev variant(`com.joynnovate.buddybird.dev`)·dev Firebase 유지.
