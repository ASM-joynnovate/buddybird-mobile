...
> **E2E는 PR 머지 전 CI 게이트가 아니다** (2026-09-10): EAS Free 플랜 Android 빌드 쿼터 소진으로 매 PR 클라우드 빌드를 중단했다. 앱 동작이 바뀌는 PR은 **작성자가 로컬에서 smoke를 실행하고 결과를 PR 본문에 첨부**한다(PR 템플릿 `E2E (로컬 실행 결과)` 섹션). CI는 nightly 회귀와 수동 실행만 담당한다.

- **빌드**: `eas build --profile preview --platform android --local` 로 **러너에서** APK 빌드 → 에뮬레이터 설치. `--local` 이라 EAS 클라우드 빌드 쿼터를 쓰지 않고(서명 크레덴셜만 `EXPO_TOKEN` 으로 fetch), 러너엔 JDK 17 셋업이 필요하다. preview는 `developmentClient` 미포함(JS 번들 내장)이라 **Metro 불필요** → subflow의 Metro 런처 진입 스텝(`optional: true` tapOn)이 실패 없이 자동 스킵됨 (§8.3 step 4). dev variant(`com.joynnovate.buddybird.dev`)·dev Firebase 유지.
- **트리거**: **PR 트리거 없음** — 작성자 로컬 실행 + PR 본문 첨부가 대체한다. nightly(schedule) = 전체 suite / `workflow_dispatch` = 수동(suite 선택). `E2E Smoke/Full (Android)` 는 main ruleset(`main-branch-protection`)의 required status check가 아니었으므로(필수 = Verify·PR Title·Auto Label·CodeQL) 트리거를 없애도 대기하는 PR은 없고 브랜치 보호 변경도 불필요하다.
