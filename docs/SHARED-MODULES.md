...
| Export | 시그니처 | 용도 |
|---|---|---|
| `segmentMetering` | `(samples: number[], options?: SegmentMeteringOptions) => VadSegment[]` | 정규화 amplitude(0..1, 100ms 간격) 타임라인을 발화 구간으로 분할. floor↔peak 적응형 임계 + sustain/release. `useFollowAlongVad` 가 저장 시 발화 구간 메타 산출에 사용. |
