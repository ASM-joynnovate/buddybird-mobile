...
| `useFollowAlongCapture` | `@/features/training/hooks/use-follow-along-capture` | `({ enabled, sessionId, wordId, cycle }) => void` | 따라하기 갭 VAD(`useFollowAlongVad` §6)를 세션에 배선. 발화 감지된 갭 녹음을 `phase:'learning'` 으로 로컬 캡처 스토어에 append(UI 노출 없음). `use-active-session`이 `useSessionAudioPlayer`의 `inFollowGap`으로 소비 |
...
| `useRestPhraseCapture` | `@/features/training/hooks/use-rest-phrase-capture` | `({ enabled, sessionId, wordId, cycle }) => void` | 휴식 구간 VAD(`useRestVad` §6)를 세션에 배선. 발화 1건이 파일로 확정될 때마다 `phase:'rest'` 로 같은 캡처 스토어에 append(UI 노출 없음). `use-active-session`이 `status==='running' && phase==='rest'` 로 소비 |
...
| `useFollowAlongVad` | `@/features/audio/hooks/use-follow-along-vad` | `voiceActive`, `detectionCount`, `permissionDenied` |
| `useRestVad` | `@/features/audio/hooks/use-rest-vad` | `permissionDenied` |
| `useMicPermissionBootstrap` | `@/features/audio/hooks/use-mic-permission-bootstrap` | 없음(부수효과: 앱 최초 실행 시 마이크 권한 선요청) |

> `useFollowAlongVad({ enabled, onSaved, resetKey? })`: 따라하기 무음 갭 동안(`enabled`)에만 마이크 녹음 + 경량 metering VAD(정규화 dB sustain/release, AEC 미사용). 갭 종료 시 발화가 감지됐으면 `persistRecordingFile` + `segmentMetering` 후 `onSaved(file, segments)` 호출. playback 모드 복귀는 호출부(재생 루프)가 소유하므로 이 훅은 stop 만. 세션 배선은 `useFollowAlongCapture`(§5) 를 소비하세요 — 이 훅을 화면에서 직접 들지 않습니다.
>
> `useRestVad({ enabled, onSaved })`: 휴식 구간 동안(`enabled`) **롤링 레코더**로 "발화 1회 = 파일 1개" 를 캡처. expo-audio 는 링버퍼가 없고 onset 은 300ms 뒤에야 확정되므로, 휴식 내내 녹음하면서 청크를 갈아끼운다 — 무음이 `ROLL_SILENCE_SAMPLES`(§6.5) 만큼 이어지면 청크를 버려 선행 무음을 제한하고, 발화가 끝나면(release) 그 청크를 파일로 확정해 `onSaved(file, segments)` 호출. **롤은 오디오 모드를 건드리지 않는다** — 휴식 중엔 재생이 없으므로 record 모드 진입은 시작 시 한 번, playback 복원은 teardown 에서 한 번. 시작·롤·teardown 은 하나의 promise 체인으로 직렬화되어, teardown 의 stop 이 진행 중인 롤의 `record()` 뒤에 오도록 보장한다(마이크 잔류 방지). 세션 배선은 `useRestPhraseCapture`(§5) 를 소비하세요.
>
> 두 VAD 훅 모두 판정 산술은 §6.3 `vad-detector` 를 씁니다. 다만 `useRestVad` 는 판정을 metering 이펙트가 아니라 **고정 100ms 인터벌**에서 돌립니다 — metering 값이 폴링 간 동일하면 이펙트가 재실행되지 않아 무음 카운터가 멈추기 때문입니다(무음 롤이 걸리지 않음).
>
...
### 6.2 오디오 pitch 프로필 해석 — `features/audio/pitch-profile.ts`

"pitch 프로필 id → 실제 재생 파라미터"의 single source of truth. 도메인 타입(`WordEntry` 등)은 불투명한 `pitchProfileId` 참조만 들고, 배속 등 실제 pitch 파라미터 해석은 오디오 도메인인 이 모듈이 소유합니다.

| Export | 시그니처 | 용도 |
|---|---|---|
| `resolvePitchPlaybackRate` | `(profileId?: string) => number` | profileId 를 재생 배속으로 해석. 미지정/미등록 id 는 `DEFAULT_PLAYBACK_RATE`(1.0, 원본 속도). `word-row` 가 미리듣기 배속으로 소비. |
| `MVP_PITCH_PROFILE` | `{ id; playbackRate; preservesPitch }` (const) | MVP 단일 pitch 프로필. 재녹음 시 `WordEntry.pitchProfileId` 에 `MVP_PITCH_PROFILE.id` 를 부여. |
| `DEFAULT_PLAYBACK_RATE` | `1.0` | pitch 프로필 미지정/미등록 시 기본 배속(원본). |
| `createMvpPitchTransform` | `(appliedAt: string) => PitchTransformMetadata` | 오디오 도메인 `PitchTransformMetadata` 생성(training 스냅샷 등 audio 도메인 소비처용). |

### 6.3 실시간 VAD 판정 — `features/audio/vad-detector.ts`
...
> 사후 파일 분할은 §6.4 `segmentMetering` 의 책임입니다. 두 모듈은 임계 기준이 유사하지만 별개로 튜닝됩니다(스트리밍 판정 vs 녹음별 적응형 분할).

### 6.4 VAD 세그먼테이션 — `features/audio/vad-segmentation.ts`

침묵/에너지 텀 기준으로 metering 타임라인을 발화 구간으로 쪼개는 순수 util. import 없음.

| Export | 시그니처 | 용도 |
|---|---|---|
| `segmentMetering` | `(samples: number[], options?: SegmentMeteringOptions) => VadSegment[]` | 정규화 amplitude(0..1, 100ms 간격) 타임라인을 발화 구간으로 분할. floor↔peak 적응형 임계 + sustain/release. `useFollowAlongVad` / `useRestVad` 가 저장 시 발화 구간 메타 산출에 사용. |
| `VadSegment` (interface) | `{ id; startMs; endMs; peaks: number[] }` | 분할된 발화 구간. `peaks` 는 막대 렌더용 다운샘플. |
| `SegmentMeteringOptions` (interface) | `{ threshold?; thresholdRatio?; minRange?; sustainSamples?; releaseSamples?; minSegmentMs?; peakBars? }` | 분할 튜닝값 override(미지정 시 녹음별 적응형). |

### 6.5 VAD 녹음·롤링 설정 — `features/audio/vad-config.ts`

| Export | 시그니처 | 용도 |
|---|---|---|
| `VAD_RECORDING_OPTIONS` | `RecordingOptions` | `HIGH_QUALITY` + `isMeteringEnabled`. 두 VAD 훅 공용 — 훅에서 프리셋을 다시 선언하지 마세요. (단어 녹음용 `useAudioRecording` 은 목적이 달라 별도 유지) |
| `ROLL_SILENCE_SAMPLES` | `number` (30 ≈ 3.0초) | `useRestVad` 롤링 레코더의 무음 롤 임계. 저장되는 발화 파일이 앞에 달고 갈 무음의 상한. 키우면 파일이 무거워지고, 줄이면 네이티브 stop/prepare 왕복이 잦아진다. |

## 7. 포맷터 — `features/shared/`

| Export | 경로 | 시그니처 | 용도 |
|---|---|---|---|
| `formatDurationSecs` | `@/features/shared/duration-format` | `(secs: number) => string` | 초 → 한국어 `X초` (<60) / 정확히 분이면 `formatDurationMins` 위임 / 잔여 초가 있으면 `<분 표기> Y초`. 분 반올림으로 잔여 초를 버리지 않음 |
| `formatDurationMins` | `@/features/shared/duration-format` | `(mins: number) => string` | 분 → 한국어 `X분` (<60) / `X시간` (정수) / `X시간 Y분` |
| `formatDurationCompact` | `@/features/shared/duration-format` | `(secs: number) => string` | 초 → 가장 큰 단위 하나로 컴팩트 표기 (칩/배지용). `X초`(<60초) / `X분`(<60분, 잔여 초 버림) / `X시간`(잔여 분 버림) |
