...
| Export | 경로 | 시그니처 | 용도 |
|---|---|---|---|
| `ageMonthsFromBirthDate` | `@/features/profile/profile-age` | `(birthDate: string \| null, nowMs?: number) => number \| null` | birthDate → 만 개월 수 (미래면 0, `null`(모름)이면 `null`). 표시(`formatAgeMonths`와 조합)·분석(`parrot_age_months`)에서 사용 |
...
| `SpeciesChips` | `@/components/profile/species-chips` | `{ selectedId: string; customActive: boolean; onSelectPreset; onCustom }` | 종을 소/중/대 그룹 칩으로 배치 + '직접입력' 칩. `getSpeciesGroups`를 자체 조회. 온보딩·프로필 편집 폼이 공유 |
