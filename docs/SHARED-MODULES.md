...
| `useSessionReplayPause` | `@/features/analytics/hooks/use-session-replay-pause` | `(): void` | 마운트 동안 세션 리플레이(Clarity 캡처) 중단 — 무거운 화면의 OOM 방지 (BB-276). consent-off 상태는 뒤집지 않음 |
...
| `loadFollowAlongCaptures` / `appendFollowAlongCapture` / `markCaptureUploaded` | `@/features/training/follow-along-capture-storage` | `() => Promise<FollowAlongCaptureStore>` / `(c: Omit<FollowAlongCapture,'sizeBytes'>) => Promise<void>` / `(id: string) => Promise<void>` | `persistKeyedStore` 기반 캡처 로컬 스토어(`@buddybird/follow-along-captures`, `capturesById.uri` 오디오 URI 컬렉션). append 는 파일 크기를 측정해 기록하고, 총량이 500MB 를 넘으면 `capturedAt` 오래된 순으로 파일·기록을 삭제해 상한 이하로 맞춘다. append/mark 는 모듈 write-queue 로 직렬화. 손상 시 조용히 초기화(학습 스토어의 throw-to-brick 와 반대 — best-effort 사이드 채널) |
...
| `uploadFollowAlongCapture` / `flushPendingFollowAlongCaptures` | `@/features/training/follow-along-upload` | `(p: FollowAlongUploadPayload) => Promise<UploadOutcome>` / `() => Promise<void>` | 캡처 백엔드 전송 seam. 현재 엔드포인트 미설정 → no-op(`skipped-no-backend`, 로컬 보관 유지). 연결 시 `FOLLOW_ALONG_UPLOAD_ENDPOINT` 설정 + multipart 전송부만 채우면 됨 |
