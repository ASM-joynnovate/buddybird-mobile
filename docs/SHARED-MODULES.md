...
| `SESSION_PERF_THRESHOLDS` / `isDegraded` / `computeTickDriftMs` / `createThrottleState` / `shouldEmit` | `@/features/training/session-perf-model` | 상수 / `(kind, valueMs) => boolean` / `(expectedAtMs, actualAtMs) => number` / `() => SessionPerfThrottleState` / `(state, kind, nowMs) => { emit, next }` | 세션 성능 저하 판정·스로틀 순수 모듈 (BB-285, 이벤트 정의 SSoT는 티켓). 기준치 200/200ms·틱 100ms·스로틀(kind별 간격 5초·세션당 상한 20건)은 잠정값 — NFR-01대로 출시 후 실측 튜닝. 소비자는 `use-session-perf` 훅 단일 |
