...
| `requestWordUpload` / `requestWordUploadFlush` | `@/features/word-library/word-upload` | `(wordId: string) => void` / `() => void` | 사용자 생성 단어 업로드의 두 진입점 (SPEC-0003 §단어 업로드). `requestWordUpload` 는 트리거 ①(단어 생성)용으로 그 1건만, `requestWordUploadFlush` 는 트리거 ②③(콜드 스타트·동의 granted 전환)용으로 미처리 전체를 보낸다. 둘 다 fire-and-forget single-flight — 진행 중 호출은 종료 후 1회 재기동 예약이며 대상이 다르면 전체로 합친다. 게이트(동의·익명 uid·`extra.apiBaseUrl`) 판정은 캡처와 같은 `isUploadGateOpen` 을 쓰고, 대상 선별·전송·응답 해석은 내부 소관이라 호출부는 시점만 책임진다. 클립 업로드와는 독립 동작 — 순서를 조율하지 않는다. 내부 구성은 역할 모듈 5개 — `word-upload-target`(대상 선별·트리거 대상 병합)·`word-upload-request`(URL·필드·파일 파트 구성과 계약 상한 절단)·`word-upload-response`(응답·실패 분기 해석)는 순수, `word-upload-client`(기기 정보 조회·multipart 전송)·`word-upload-state`(처리 기록)만 I/O. 웹 no-op |
| `loadWordUploadState` / `markWordUploadResult` | `@/features/word-library/word-upload-state` | `() => Promise<WordUploadState>` / `(clientWordId: string, status: 'uploaded' \| 'failed') => Promise<void>` | 단어 업로드 처리 기록(`@buddybird/word-upload-state`). 단어 데이터에 업로드 필드를 더하지 않기 위해 저장소를 분리했다. `failed` 는 4xx 거부라 재전송하지 않으며, 깨진 기록은 빈 상태로 되돌린다 — 서버가 `(firebase_anon_uid, client_word_id)` 멱등이라 전량 재전송해도 중복 행이 생기지 않는다 |
