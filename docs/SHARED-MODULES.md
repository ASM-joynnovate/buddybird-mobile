...
| `AnalyticsContextValue.track` | `@/features/analytics/analytics-context` | `<E extends AnalyticsEvent>(event: E \| Promise<E \| null>, recoveryEvent?: AnalyticsEvent \| Promise<AnalyticsEvent \| null>) => void` | SDK·동의·초기 프로필 속성 준비 후 등록 순서로 전송. 비동기 파라미터는 Promise로 즉시 예약하고, null은 예약 취소. recoveryEvent는 프로세스 재시작 시 복구 가능한 최소 이벤트 |
...
| `trackEvent` | `@/features/analytics/event-tracker` | `<E extends AnalyticsEvent>(event: E) => void` | React 밖(순수 모듈)에서 이벤트 발행 — 훅을 못 쓰는 업로드 파이프라인용. provider 등록 전 메모리에 대기시키고 등록 후 공통 로컬 저장 대기열로 전달. 컴포넌트·훅에서는 `useAnalytics().track` 을 쓴다 |
| `analyticsOutboxStore` | `@/features/analytics/event-outbox` | `AnalyticsOutboxStore` | 이벤트별 로컬 저장과 발생 순서 복원. `load()`로 복원, `write(changed, removedIds)`로 변경 항목과 삭제만 저장 |
| `parseAnalyticsOutbox` | `@/features/analytics/event-outbox` | `(raw: unknown) => AnalyticsOutboxEntry[]` | 속성, UID, 발생 시각, provider별 성공 기록 검증. 손상 데이터는 throw, 미완성 Promise 예약은 recoveryEvent로 복구하거나 정보가 없으면 경고 후 해제 |
...
| `SessionStatus` (type) | `@/features/training/session-config` | `SessionEngineState` | 네이티브 세션 엔진과 동일한 세션 상태 |
| `SessionMeta` (interface) | `@/features/training/session-config` | meta payload | 세션 시작 시 저장되는 메타 |
| `CATS` | `@/features/word-library/word-categories` | `readonly WordCategory[]` | 단어 필터 카테고리 키 목록 (`all` + `WORD_TAGS`) — 표시 라벨은 i18n `wordLibrary.filterAll`/`tagLabels.*` 해석 |
| `WordCategory` (type) | `@/features/word-library/word-categories` | `'all' \| WordTag` | 단어 필터 카테고리 키 타입 |
| `requestWordUpload` / `requestWordUploadFlush` | `@/features/word-library/word-upload` | `(wordId: string) => void` / `() => void` | 사용자 생성 단어 업로드의 두 진입점 (SPEC-0003 §단어 업로드). `requestWordUpload` 는 트리거 ①(단어 생성)용으로 그 1건만, `requestWordUploadFlush` 는 트리거 ②③(콜드 스타트·동의 granted 전환)용으로 전체를 보낸다. 둘 다 fire-and-forget single-flight — 진행 중 호출은 종료 후 1회 재기동 예약이며 대상이 다르면 전체로 합친다. 게이트(동의·익명 uid·`extra.apiBaseUrl`) 판정은 캡처와 같은 `isUploadGateOpen` 을 쓰고, 대상 선별·전송·응답 해석은 내부 소관이라 호출부는 시점만 책임진다. 클립 업로드와는 독립 동작 — 순서를 조율하지 않는다. 첫 배포에는 처리 기록을 두지 않아 전송 여부와 무관하게 매번 전량을 보낸다 (BB-238 결정) — 서버가 `(firebase_anon_uid, client_word_id)` 멱등이라 중복 행이 생기지 않으며, 재전송을 거르는 규칙은 다음 업데이트 소관이다. 내부 구성은 역할 모듈 4개 — `word-upload-target`(대상 선별·트리거 대상 병합)·`word-upload-request`(URL·필드·파일 파트 구성과 계약 상한 절단)·`word-upload-response`(응답·실패 분기 해석)는 순수, `word-upload-client`(기기 정보 조회·multipart 전송)만 I/O. 웹 no-op |
...
| `deriveSessionCycles` | `@/features/training/session-cycle-model` | `({ totalSeconds, learnSecs, restSecs, careSecs }) => { secsPerCycle, totalCycles, totalSessionSeconds, totalLearningSeconds, sessionMins }` | 세션 cycle 파생 단일 소스. 사이클 = 학습+휴식+스트레스 케어 (BB-380). 분 단위 호출부는 `mins * 60`을 `totalSeconds`로 넘김. `secsPerCycle<=0`이면 `totalCycles=1` |
...
| `completedCyclesAtPosition` | `@/features/training/session-cycle-model` | `(cycle, phase, phaseElapsed, restSecs, careSecs) => number` | 부분 마지막 회차를 포함한 네이티브 위치에서 완전히 끝낸 회차 수 계산. 사이클 완료 기준 = 마지막 구간(care가 있으면 care, 없으면 rest) 종료 |
| `SessionCyclePhase` (type) | `@/features/training/session-cycle-model` | `'learning' \| 'rest' \| 'stress-care'` | 세션 사이클 구간 타입 — 네이티브 엔진 스냅샷 phase 문자열과 1:1 |
| `STRESS_CARE_SECONDS` | `@/features/training/session-config` | `number` (300) | 스트레스 케어 구간 길이 (BB-380, PRD FR-07 사이클당 5분) |
| `STRESS_CARE_TRACK_MODULES` | `@/features/training/stress-care-tracks` | `number[]` | 스트레스 케어 온디바이스 번들 트랙(HE-AAC 48kbps 5분 × 3) require 레지스트리 — 구간별 랜덤 추첨은 네이티브 엔진 소관, JS는 `prepareSessionAudioUri`로 URI만 준비해 전달 |
...
| `SESSION_PERF_THRESHOLDS` / `isDegraded` / `computeTickDriftMs` / `createThrottleState` / `shouldEmit` / `observeAudioDelay` | `@/features/training/session-perf-model` | 상수 / `(kind, valueMs) => boolean` / `(expectedAtMs, actualAtMs) => number` / `() => SessionPerfThrottleState` / `(state, kind, nowMs) => { emit, next }` / `(lastObserved, observed) => { nextObserved, confirmedDelayMs }` | 세션 성능 저하 판정·스로틀·audio_delay 전이 판정 순수 모듈 (BB-285, 이벤트 정의 SSoT는 티켓). 기준치 200/200ms·틱 100ms·스로틀(kind별 간격 5초·세션당 상한 20건)은 잠정값 — NFR-01대로 출시 후 실측 튜닝. 소비자는 `use-session-perf` 훅 단일 |
...
| `sessionAudioEngine` | `@/modules/session-audio-engine` | `SessionAudioEngine` | 단조 시간, 구간 전환, 백그라운드 재생·연속 마이크 입력, 진행 기록과 복구를 소유하는 로컬 Expo 모듈 |
...
| `useMicPermissionBootstrap` | `@/features/audio/hooks/use-mic-permission-bootstrap` | 없음(부수효과: 앱 최초 실행 시 마이크 권한 선요청) |
>
> `useMicPermissionBootstrap()`: root layout(`MicPermissionBootstrap`)에서 앱 최초 실행 시 마이크 권한을 선요청. 상태가 `undetermined` 일 때만 요청하므로 시스템 다이얼로그는 설치 후 1회만 뜨고, 이미 허용/거부된 뒤에는 재요청하지 않습니다. VAD·녹음 진입점의 lazy 요청은 그대로 유지되어(거부 후 재요청 안전망) 이 훅과 독립적으로 동작합니다.
...
| `getRecordingFileSize` | `(uri?: string \| null) => number` | 녹음 파일 크기(bytes). 파일 없음·오류 시 0. 저장형/절대 URI 모두 처리. 캡처 보관 상한 계산에 사용. |
| `deleteRecordingFile` | `(uri?: string \| null) => void` | 녹음 파일 삭제(idempotent). 없거나 실패해도 비치명. 캡처 보관 상한 초과 시 오래된 파일 정리에 사용. |
...
> 사용 의무: `features/word-library/word-library-storage.ts`, `features/training/training-storage.ts` 등 AsyncStorage 계층은 `persistKeyedStore`의 `audioUriCollections`로 오디오 URI 컬렉션·필드를 선언해야 합니다. 필드명은 `.` 로 중첩 경로를 선언할 수 있습니다 (예: `pitchTransform.transformedUri`) — `pitchTransform.transformedUri` 같은 legacy 중첩 URI도 stale 없이 정규화/hydration 됩니다. `normalizeAudioUriForStorage` / `hydrateAudioUriFromStorage`는 seam이 자동 소유하므로 storage 모듈에서 직접 호출하지 않습니다. UI/도메인 코드는 in-memory 절대 URI를 그대로 사용합니다 ([CONVENTIONS §6 참고](./CONVENTIONS.md#6-데이터-영구화-storage)).

### 6.2 실시간 VAD 판정 — `features/audio/vad-detector.ts`

스트리밍 metering 샘플(100ms 폴링) 하나씩으로 발화 onset/release 만 판정하는 순수 util. import 없음. 튜닝 상수의 single source of truth — VAD 훅이 임계값·sustain/release 를 자체 선언하지 않습니다.

| Export | 시그니처 | 용도 |
|---|---|---|
| `normalizeMetering` | `(db: number) => number` | metering dB(−60..−10)를 0..1 진폭으로 정규화. |
| `createVadState` | `() => VadState` | `{ aboveRun: 0, belowRun: 0, isVoice: false }` 초기 상태. |
| `stepVad` | `(state: VadState, normalized: number) => 'onset' \| 'release' \| null` | 샘플 하나로 state 를 제자리 전진시키고 경계 전이를 반환. 임계 초과 샘플에서 `belowRun` 이 0으로 리셋되므로 발화 시작 틱에서는 무음 카운터 기반 동작이 개시되지 않는다. |
| `VAD_THRESHOLD` / `VAD_SUSTAIN_SAMPLES` / `VAD_RELEASE_SAMPLES` | `0.35` / `3` / `5` | 정규화 임계값 · onset(≈300ms) · release(≈500ms) 샘플 수. |
| `VadState` (interface) | `{ aboveRun; belowRun; isVoice }` | 훅이 ref 로 들고 다니는 가변 판정 상태. |
...
| Export | 경로 | 시그니처 | 용도 |
|---|---|---|---|
| `formatDurationSecs` | `@/features/shared/duration-format` | `(secs: number, t: Translate) => string` | 초 → 로케일 `X초` (<60) / 정확히 분이면 `formatDurationMins` 위임 / 잔여 초가 있으면 `<분 표기> Y초`. 분 반올림으로 잔여 초를 버리지 않음. `t`는 호출부가 `useI18n()`에서 꺼내 주입 (`common.duration.*` 키 사용) |
| `formatDurationMins` | `@/features/shared/duration-format` | `(mins: number, t: Translate) => string` | 분 → 로케일 `X분` (<60) / `X시간` (정수) / `X시간 Y분` |
| `formatDurationCompact` | `@/features/shared/duration-format` | `(secs: number, t: Translate) => string` | 초 → 가장 큰 단위 하나로 컴팩트 표기 (칩/배지용). `X초`(<60초) / `X분`(<60분, 잔여 초 버림) / `X시간`(잔여 분 버림) |
...
| `BuddyBird` | `@/components/mascot/buddy-bird` | `{ size?, animation? }` | 재사용 가능한 버디 마스코트 (3D PNG asset: `assets/images/buddy-bird.png`). reduce-motion에서 정적 렌더 |
...
### 9.2 파형 metering 계산 — `components/ui/waveform-metering.ts`

정규화된 마이크 metering(0..1)을 파형 막대 높이로 변환하는 순수 계산부. `WaveformPlaceholder`(녹음 편집 카드)와 `WaveformBars`(녹음 생성 카드)가 공유 — 마이크 반응 파형 로직의 single source. 도형(막대 개수·픽셀 높이)은 소비 컴포넌트가 소유하고, 여기서는 0..1 정규화 값만 반환한다(geometry-agnostic).

| Export | 시그니처 | 용도 |
|---|---|---|
| `WAVEFORM_NOISE_FLOOR` | `number` (0.25) | 이 값 미만 metering 은 배경 잡음으로 간주. |
| `meteringEffectiveLevel` | `(metering: number) => number` | 잡음 바닥 제거 후 그 이상 구간을 0..1 로 재정규화한 유효 레벨. |
| `meteringBarLevel` | `(effective: number, index: number, barCount: number) => number` | 유효 레벨을 막대 index 별 0..1 목표 높이로 변환 — 가운데 가중 + 약한 지터. |

> **예외 기록**: `WheelPicker`는 등재 시점에 단일 caller(`session-preset-card.tsx`) 상태로 추출됨. 절차 §1-5(2회 이상 중복 시 추출) 예외. 사유: UI primitive로서 향후 다른 숫자 선택 surface 재사용 잠재력. 출처: `~/.claude/plans/hidden-scribbling-sunbeam.md` — BB-234에서 `BirthDatePicker`(생년월일 드럼롤)가 2번째 소비처가 되어 예외 해소.
...
## 11. 프로필 입력 (나이·생년월일·종) — `features/profile/`, `components/profile/`

프로필 나이는 `birthDate`(YYYY-MM-DD, `null` = 모름)를 정본으로 저장하고, 표시·분석용 나이(개월)는 저장하지 않고 항상 birthDate에서 파생한다. 레거시(생년월일 도입 전) 프로필의 `ageMonths` 스냅샷은 `profile-storage`가 로드 시 `createdAt` 기준으로 역산 back-fill 한다 (지연 마이그레이션). 저장 계층은 `birthDate: null`(신규 '모름')과 `birthDate` 미보유(레거시 → 역산)를 구분한다. 종(species)은 몸집 소/중/대 그룹으로 배치하며, 프리셋 목록은 `SPECIES_SIZE_GROUPS`(profile-options 내부)에서 파생한다.

| Export | 경로 | 시그니처 | 용도 |
|---|---|---|---|
| `ageMonthsFromBirthDate` | `@/features/profile/profile-age` | `(birthDate: string \| null, nowMs?: number) => number \| null` | birthDate → 만 개월 수 (미래면 0, `null`(모름)이면 `null`). 분석(`parrot_age_months` 이벤트 param·user property)에서 사용 |
| `birthDateFromAgeMonths` | `@/features/profile/profile-age` | `(createdAtIso: string, ageMonths: number) => string` | 레거시 마이그레이션 역산: createdAt − ageMonths 달의 1일을 생일로. `profile-storage`가 사용 |
| `parseBirthDate` / `formatBirthDate` | `@/features/profile/profile-age` | `(birthDate: string) => { year, month, day }` / `(year, month, day) => string` | YYYY-MM-DD 파싱·조립 |
| `daysInMonth` | `@/features/profile/profile-age` | `(year: number, month: number) => number` | 해당 연·월의 말일. 드럼롤 일(day) 옵션·클램프 |
| `BirthDatePicker` | `@/components/profile/birthdate-picker` | `{ value: string; onChange: (birthDate: string) => void }` | 년/월/일 3컬럼 드럼롤(값은 non-null). `WheelPicker`를 조합. `BirthDateField`가 사용 |
| `BirthDateField` | `@/components/profile/birthdate-field` | `{ label; value: string \| null; onChange: (v: string \| null) => void; error?; labelStyle? }` | 생년월일 입력 필드 — 라벨 행 우측에 '모름' 토글(`null`), 아래에 드럼롤(값 있음)/안내(모름). `FormField`(`labelAccessory`)와 `BirthDatePicker`를 조합. 온보딩·프로필 편집 폼이 공유 |
| `getSpeciesGroups` | `@/features/profile/profile-options` | `(locale) => SpeciesGroup[]` | 프리셋 종을 소/중/대(`SpeciesSize`) 그룹 + 로케일 라벨로 반환. `PRESET_SPECIES_IDS`는 이 그룹에서 파생 |
| `SpeciesChips` | `@/components/profile/species-chips` | `{ selectedId: string; onSelectPreset: (id: string) => void }` | 종을 소/중/대 그룹 칩으로 배치. '직접입력' 토글은 별도 `CustomSpeciesChip`(FormField `labelAccessory`)이 소유. `getSpeciesGroups`를 자체 조회. 온보딩·프로필 편집 폼이 공유 |
