...
| `useFollowAlongCapture` | `@/features/training/hooks/use-follow-along-capture` | `({ enabled, sessionId, wordId, cycle }) => void` | 따라하기 갭 VAD(`useFollowAlongVad` §6)를 세션에 배선. 발화 감지된 갭 녹음을 로컬 캡처 스토어에 append(UI 노출 없음). `use-active-session`이 `useSessionAudioPlayer`의 `inFollowGap`으로 소비 |
