...
> **2026-07-13 현행화:** BB-215가 이 ADR의 초기 Auth 시점을 일부 대체했다. 앱은 이제 시작할 때
> 공용 Firebase 익명 uid를 복원·발급하고, 피드백 문서의 `userId`에 같은 uid를 저장한다. Firestore
> 채택과 create-only 결정은 그대로 유효하다.
...
위 결정 중 "의견을 보낼 때만 익명 로그인" 정책은 BB-215와 PR #105에서 eager 로그인으로
대체됐다. 앱은 시작할 때 공용 Firebase 익명 uid를 복원·발급하고, 피드백 문서의 `userId`에 같은
uid를 저장한다. 첫 실행이 오프라인이면 uid 없이 부팅하되 foreground에서 재시도하고, 제출
직전에도 uid를 방어적으로 확인한다. Firestore 채택과 create-only 결정은 그대로 유효하다.
...
- Firestore 켜기·익명 로그인 켜기·보안 규칙 배포는 앱 코드 밖(Firebase 콘솔) 작업이다.
  Security Rules의 운영 정본도 콘솔이며 레포에는 `firestore.rules`를 두지 않는다. 절차는
  `docs/feedback-backend.md` 참고.
