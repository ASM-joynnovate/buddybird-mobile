...
대기 이벤트와 당시 사용자 속성, UID, 발생 시각은 항목별 로컬 저장소에 보관한다. SDK 초기화나 속성 적용이 실패하면 실패한 provider만 발생 순서대로 재시도한다. 재시도 간격은 1초부터 최대 60초까지 증가하며, 앱 foreground 복귀 시 즉시 재시도한다. SDK 호출이 끝나지 않으면 중복 호출을 시작하지 않고 대기하며, 앱 재시작 시 저장된 항목을 복원한다.

- A 시점 이벤트에는 A 속성과 UID 적용. 이후 B 변경은 그 이후 이벤트부터 적용
- 과거 A 이벤트 복구 후에는 현재 B 속성과 UID 복원. 전송 실패 경로에서도 복원 시도
- UID 미확보 상태의 초기 이벤트는 최초 전송 준비 시 확보한 UID 적용. 이미 UID가 확정된 이벤트는 복구 시에도 당시 UID 유지
- provider별 SDK 호출 성공 기록을 저장하고, 성공한 provider에는 같은 이벤트 재전송 생략
- 이전 client의 진행 중 저장 완료 후 새 client 복원. 이전 SDK 상태 변경 완료 후 새 SDK 상태 적용. 지연된 이전 저장으로 최신 기록을 덮어쓰는 동작 방지
- 동의 조회 실패 시 보관과 조회 재시도 유지. 명시적 수집 거부 시 대기 기록 삭제
- 초기화 대기 중에도 앱 시작, background, 온보딩 이탈 이벤트 등록. 업데이트 확인도 Analytics 준비와 독립 실행

로컬 저장 완료 전 SDK 호출을 시작하지 않는다. 보관 개수나 기간에 따른 자동 폐기는 하지 않으며, 저장 공간이 부족하거나 저장이 실패하면 경고와 재시도로 처리한다. 저장 완료 전 프로세스가 종료된 항목은 복구를 보장할 수 없다. 파라미터 계산 중인 Promise 자체는 복원할 수 없다. 학습 시작이 네이티브에서 확인되면 복구용 시작 정보를 먼저 저장한다. 누계 조회 중 프로세스가 종료되면 시작 이벤트를 순서대로 복구하되 미확정 누계 필드는 생략하고 `client_event_recovered=true`로 표시한다. 성공 확인이나 복구 정보 저장이 끝나기 전에 종료된 예약, 복구 정보가 없는 다른 비동기 예약은 경고 후 해제한다.

SDK 수락 직후 성공 기록을 저장하기 전에 프로세스가 종료되면 중복 전송될 수 있다. `client_event_id`와 `client_event_time_ms`를 일반 이벤트와 Firebase 수동 화면 이벤트의 파라미터에 포함해 같은 이벤트와 실제 발생 시각을 구분한다. 이 파라미터가 GA의 자동 중복 제거를 제공하거나 기본 이벤트 시각을 소급 수정하지는 않는다. SDK 전달 완료는 GA 서버 수신 완료가 아니다. 장기간 보관 시 보고서 날짜와 세션 분류에 영향을 줄 수 있다.
...
### 대기 이벤트 복구
- 초기화 실패와 지연 중 이벤트를 로컬 저장하고 복구 시 발생 순서대로 전송
- 속성과 UID 변경은 이후 이벤트부터 적용하며, 이전 비동기 처리 결과로 최신 상태 덮어쓰기 금지
- 저장 실패와 SDK 실패가 프로필 저장과 화면 처리를 대기시키지 않도록 분리
