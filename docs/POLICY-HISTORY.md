...
| 2026-06-07 | conversation (release-please 오작동 분석 + RN 배포 자동화 조사) | **deprecated: release-please 기반 자동 semver** → 수동 bump + 버전 게이트 직접 릴리즈 파이프라인으로 전환. 배경: (1) `target-branch` 미지정으로 release PR이 default 브랜치(dev)에 생성되는 오작동 (PR #64), (2) squash-merge 환경에서 squash 본문 불릿(`* feat:`) 미파싱 → promote PR마다 `BEGIN_COMMIT_OVERRIDE` 수동 작성 의존, (3) 릴리즈당 main 머지 2회 필요. star 수 상위 RN 오픈소스 앱(Bluesky·Expensify·Mattermost·RocketChat) 조사 결과 conventional-commit 자동 semver 도구 채택 0곳 — 수동 bump PR + 명시적 트리거가 실무 표준. 신규 구조: `yarn release:bump <patch\|minor\|major>` 커밋을 promote cascade에 포함 → main push 시 `eas-production.yml` `release-gate`가 package.json version과 기존 태그 비교 (이미 태그된 버전이면 빌드 skip) → 빌드 성공 후 tag `vX.Y.Z` + GitHub Release(`--generate-notes`) 자동 생성. CHANGELOG.md 파일 폐지 (GitHub Releases가 릴리즈 노트 단일 소스), commit type→semver 자동 매핑 폐지 (type은 PR title gate + bump 수준 판단 근거로 유지), 핫픽스는 `release:bump patch` 커밋 포함 의무 | `.github/workflows/release-please.yml`·`release-please-config.json`·`.release-please-manifest.json`·`CHANGELOG.md` 삭제, `.github/workflows/eas-production.yml` 재작성, `package.json` (`release:bump` 스크립트), `.github/labeler.yml`, `docs/BUILD-AND-RELEASE.md` §12.1·§12.2·§12.5·§12.6·§12.11·§13 Hard Rules, `docs/CONVENTIONS.md` §4.1.1, `CLAUDE.md` Hard Rules·브랜치 전략 |
| 2026-06-20 | issue `.scratch/architecture-deepening/issues/03-audio-uri-seam-retire-grep-gate.md` (architecture review 후보 1, slice #03) | **deprecated: §5 오디오 URI normalize/hydrate grep 게이트 + storage 수기 호출 의무 폐기** — 모든 store(`word-library`/`training`/`word-metrics`)가 `persistKeyedStore` seam의 선언형 `audioUriCollections`를 경유하도록 이관 완료(#01/#02)되어, 변환 invariant가 seam 인터페이스 차원에서 강제됨. "각 *-storage.ts가 normalize/hydrate를 거치는지" 검사하던 §5 grep 게이트는 무의미해져 제거. 변환은 seam이 소유하며 storage/호출자는 normalize/hydrate를 직접 호출하지 않음. `normalizeAudioUriForStorage`/`hydrateAudioUriFromStorage`/`normalizeAudioUriFields`/`hydrateAudioUriFields`는 seam 내부 primitive로 존속. CI엔 해당 grep 게이트 없었음(`_verify.yml`은 `npx expo install --check`만 수행) — 다른 검증 grep(rgba/empty-catch/namespaced firebase)·줄 수 예산 게이트는 유지 | `docs/CONVENTIONS.md` §5·§6.1, `docs/SHARED-MODULES.md` §6.1 |
