...
- Conventional commits 소문자 + scope, `yarn lint && yarn typecheck` 그린 후에만 커밋 — 정확한 type 필수 (PR title gate + 릴리즈 bump 수준 판단 근거, `docs/BUILD-AND-RELEASE.md` §12.2)
...
- `package.json` / `app.config.ts` 의 `version` 필드 손편집 금지 — `yarn release:bump` 스크립트로만 변경 (릴리즈 promote 시, `docs/BUILD-AND-RELEASE.md` §12.2)
...
- `dev` → `staging`: PR merge 시 개발계 자동 출시 (Android Play internal track + iOS TestFlight Internal)
- `staging` → `main`: PR merge → 버전 게이트 통과 시 운영 빌드 자동 → 성공 시 tag + GitHub Release 자동 생성, 출시는 수동 promote (릴리즈 전 `yarn release:bump` 필수, `docs/BUILD-AND-RELEASE.md` §12.2·§12.5)
- `hotfix/*` → `main`: 긴급 패치, staging 우회 가능. `yarn release:bump patch` 커밋 포함 의무. merge 후 `main → staging → dev` back-merge **(핫픽스 한정 의무, `dev` 까지 전파해야 version 충돌 재발 방지)** (단일 semver + buildNumber 분리 패턴, `docs/BUILD-AND-RELEASE.md` §12.2·§12.6)
...
Five canonical triage roles, default names (`needs-triage` / `needs-info` / `ready-for-agent` / `ready-for-human` / `wontfix`), applied as the `Status:` line in each local issue file. See `docs/agents/triage-labels.md`.
