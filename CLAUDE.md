...
- Conventional commits 소문자 + scope, `yarn lint && yarn typecheck` 그린 후에만 커밋 — 정확한 type 필수 (PR title gate + 릴리즈 bump 수준 판단 근거, `docs/BUILD-AND-RELEASE.md` §12.3)
- `--no-verify` 금지, try/catch로 lint 우회 금지, attribution 라인 금지
- 보호자 PII (이름·이메일·전화·정확한 위치) 수집 금지
- iOS·Android 사용자 식별자의 정본은 Firebase 익명 Auth uid — installation UUID나 profile.id로 analytics user id를 설정하지 않음
- Firebase config 파일 (`GoogleService-Info.plist`, `google-services.json`) commit 금지
- `package.json` / `app.config.ts` 의 `version` 필드 손편집 금지 — `yarn release:bump` 스크립트로만 변경 (릴리즈 태그 직전, `docs/BUILD-AND-RELEASE.md` §12.3)
